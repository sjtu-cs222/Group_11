...
def train(data, save_model_dir, seg=True, ori_model_dir=None, use_attn=False, use_w2c=False):
    ...
    sgd_optimizer = optim.SGD(parameters, lr=data.HP_lr, momentum=data.HP_momentum)
    ...
    for idx in range(data.HP_iteration):
        ...
        if current_score > best_dev or idx == data.HP_iteration - 1:
            ...
            if idx > 3:
                save_data_setting(data, save_data_name + '.' + str(idx) + '.dset')
...
if __name__ == '__main__':
    ...
    handler = logging.FileHandler(save_dir + 'log')
    ...
    parser.add_argument('--savemodel', default="data/MSRA/test_1/saved_model")
    parser.add_argument('--train', default="data/MSRA/train.txt")
    parser.add_argument('--dev', default="data/MSRA/test.txt")
    parser.add_argument('--test', default="data/MSRA/test.txt")
    ...
    # logger.info(("CuDNN:", torch.backends.cudnn.enabled))
    # gpu = False
    logger.info(("GPU available:", gpu))
    ...
    logger.info("In config.json...")
    logger.info(("norm_gaz_emb:", config['norm_gaz_emb']))
    logger.info(("use_attn:", config['use_attn']))
    logger.info(("use_w2c:", config['use_w2c']))
    logger.info(("shared_attn:", config['shared_attn']))
    logger.info(("n_head:", config['n_head']))
    logger.info(("combination:", config['combination']))
    logger.info(("save_dir:", config['save_dir']))

    if status == 'train':
        logger.info(("Model saved to:", save_model_dir))
    sys.stdout.flush()
    ...
    if status == 'train':
        ...
        data.norm_gaz_emb = config['norm_gaz_emb']
        ...
        train(data, save_model_dir, seg, use_w2c=config['use_w2c'], use_attn=config['use_attn'])

        #
        # data = load_data_setting('data/MSRA/attn2/saved_model.dset. .dset')
        #
        # data.generate_instance_with_gaz(train_file,'train')
        # data.generate_instance_with_gaz(dev_file,'dev')
        # data.generate_instance_with_gaz(test_file,'test')
        #
        # train(data, save_model_dir, seg, 'data/MSRA/test_no_w2c/saved_model.5.model', use_attn=True, use_w2c=False)
